...
class rangeSet:
    '''
    A structure that behaves similarly to a set, but allows selecting ranges via slicing. As such, much asymptotic complexity
    is different (this uses essentially a priority queue as a backend), and behavior is subtly different from a true set; see
    method descriptions for differences. One really big difference is the
    classic len(function) - in python, slicing normally uses this function to wrap negative indices. In this case, negative
    indices should be preserved, so calling len() on a rangeSet will ALWAYS return zero. To get the length, use the rangeSet.len()
    function
    '''
    class alwaysSmaller:
    ...
    def __init__(self, fromIterable=None):
        self.myList = sortedlist()
        if fromIterable != None:
            for v in fromIterable:
                self.add(v)
    ...
    def __len__(self):
        '''
        This is actually broken; see class definition note (use rangeSet.len() instead)
        '''
        return 0
        #return len(self.myList)
    ...
    def __getitem__(self, key):
        ...
        - ALL queries will return a sorted list (there is no additional cost for the sorting; it's a natural by-product of
          ...
          would be raised)
        Complexity is m log^2 n operations or m log n comparisons, where n is the size of the whole set and m is the number
        ...
        if isinstance(key,slice):
            ...
            stop = key
            step = None
        return [v for v in self.myList[self.myList.bisect(start):self.myList.bisect_right(stop):step]]
    
    def count(self, low, high=None, step=1):
        if high == None:
            high = low
        return (self.myList.bisect(high) - self.myList.bisect(low))/step
    
    def add(self, v):
        '''
        NOTE THAT ALL VALUES MUST BE UNIVERSALLY COMPARABLE; e.g. you may not add a list object as a value.
        '''
        try:
            self.myList.add(v)
        except TypeError:
            errorstr = str("keys and values must be universally comparable: %s" % str(v))
            raise TypeError(errorstr)
    
    def discard(self, v):
        while self.has_key(key):
            del self.myList[self.myList.index(key)]
